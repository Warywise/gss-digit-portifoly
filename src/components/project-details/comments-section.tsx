...
const CommentsSection: React.FC<{ project: ProjectsModel }> = ({ project }) => {
  const { handleComment } = useUserStore(); // A função do Zustand que chama a Server Action
  ...
  const onSubmitComment = async (content: string) => {
    ...
    try {
      ...
      // Opcional: router.refresh() se o revalidateTag não for instantâneo o suficiente
    } catch (error) {
      console.error(error);
